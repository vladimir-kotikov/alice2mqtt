...
export interface AliceDevicesRequest extends AliceBaseRequest { }
...
type AliceColorSceneId = 'alarm' | 'alice' | 'candle' | 'dinner' | 'fantasy' | 'garland' | 'jungle' | 'movie' | 'neon' | 'night' | 'ocean' | 'party' | 'reading' | 'rest' | 'romance' | 'siren' | 'sunrise' | 'sunset';
...
export interface AliceColorSettingCapabilityState extends BaseAliceDeviceCapabilityState {
  type: AliceCapabilityType.ColorSetting;
  state: { instance: 'rgb' | 'temperature_k'; value: number }
  | { instance: 'scene'; value: string }
  | { instance: 'hsv'; value: { h: number; s: number; v: number } };
}

type AliceModeName = 'cleanup_mode' |
  'coffee_mode' |
  'dishwashing' |
  'fan_speed' |
  'heat' |
...
export type AliceModeValue = 'auto' | 'eco' | 'turbo' |
  ...
  'americano' | 'cappuccino' | 'double' | 'espresso' | 'espresso' | 'latte' |
...
export type AlicePercentageRangedInstanceName = 'brightness' | 'channel' | 'humidity' | 'open' | 'volume';

// FIXME: These should not overlap
export type AliceTemperatureRangedInstanceName = 'brightness' | 'channel' | 'humidity' | 'open' | 'temperature' | 'volume';
...
export type AliceRangedCapabilityInfo = AlicePercentageRangedCapabilityInfo | AliceTemperatureRangedCapabilityInfo;

interface AliceRangedCapabilityState extends BaseAliceDeviceCapabilityInfo {
  type: AliceCapabilityType.Range;
  state: {
...
export type AliceDeviceCapabilityInfo = AliceOnOffCapabilityInfo |
  AliceColorSettingCapabilityInfo |
  AliceModeCapabilityInfo |
  AliceRangedCapabilityInfo |
  AliceToggleCapabilityInfo;

export type AliceDeviceCapabilityState = AliceOnOffCapabilityState
...
type AlicePressureUnit = 'unit.pressure.atm' | 'unit.pressure.pascal' | 'unit.pressure.bar' | 'unit.pressure.mmh';
...
interface AliceFloatDevicePropertyState extends BaseAliceDevicePropertyState {
  type: AlicePropertyType.Float;
  state: {
    instance: 'battery_level' | 'co2_level' | 'humidity' | 'power' | 'pressure' | 'temperature' | 'voltage' | 'water_level';
    value: number;
  };
}
...
interface AliceEventDevicePropertyState extends BaseAliceDevicePropertyState {
  type: AlicePropertyType.Event;
  state: {
    instance: 'vibration' | 'open' | 'button' | 'motion' | 'smoke' | 'gas' | 'battery_level' | 'water_level' | 'water_leak';
    value: string;
  };
}

export type AliceDevicePropertyInfo = AliceFloatDevicePropertyInfo | AliceEventDevicePropertyInfo;
export type AliceDevicePropertyState = AliceFloatDevicePropertyState | AliceEventDevicePropertyState;
...
interface AliceDevicesResponsePayload {
  user_id: string;
  devices: AliceDeviceMetadata[];
}

export interface AliceDevicesResponse {
  request_id: string;
  payload: AliceDevicesResponsePayload;
}
