...
const HapCharacteristicType = require('../../types/hap').HapCharacteristicType;
const HapServiceType = require('../../types/hap').HapServiceType;
const enumNameByValue = require('../util').enumNameByValue;
const getCharacteristic = require('../util').getCharacteristic;

var debug = require('debug')('alice2mqtt:Service');
...
function Service(devices, context) {
    ...
    const serviceName = getCharacteristic(devices, HapCharacteristicType.ConfiguredName)
        || getCharacteristic(devices, HapCharacteristicType.Name)
        || 'No name';

    debug(
        `Reading characteristics for service ${devices.type} `
        + `(${enumNameByValue(HapServiceType, devices.type)}) (name=${serviceName})`
    );

    devices.characteristics.forEach(function (element) {
        var service = new Characteristic(element, this);
        if (element.type === HapCharacteristicType.Name) {
...
Service.prototype.getDeviceCapabilities = function (context) {
    ...
    for (var index in this.characteristics) {
        ...
        if (characteristic.type !== HapCharacteristicType.Name && characteristic.capabilities) {
...
Service.prototype.getDeviceState = function () {
    ...
    for (var index in this.characteristics) {
        ...
        if (characteristic_data.type !== HapCharacteristicType.Name && characteristic_data.capabilities) {
